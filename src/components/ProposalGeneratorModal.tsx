...
export const ProposalGeneratorModal: React.FC<ProposalGeneratorModalProps> = ({
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-sm p-3 sm:p-6 flex flex-col items-center justify-start pt-6 sm:pt-10 font-montserrat text-[#1e1b1b]">
        
        {/* Top Floating Action Bar */}
        <div className="no-print sticky top-2 z-50 bg-white border border-stone-300 rounded-2xl p-3 sm:p-4 shadow-2xl flex flex-wrap items-center justify-between gap-3 w-full max-w-4xl mb-6">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 rounded-xl bg-[#4a5d4a] flex items-center justify-center text-white shadow-md">
              <FileText className="w-5 h-5" />
            </div>
            <div>
              <h3 className="text-sm font-bold text-[#1e1b1b] uppercase tracking-tight">Generador de Propuesta Técnica Formal PDF</h3>
              <p className="text-xs text-stone-500">Documento institucional de alta calidad para presentar a clientes</p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className="flex items-center space-x-2 px-5 min-h-[44px] bg-[#4a5d4a] hover:bg-[#3b4b3b] text-white font-bold text-xs uppercase tracking-wider rounded-sm shadow-md transition cursor-pointer"
            >
              {isGeneratingPdf ? (
            ...
            <button
              onClick={handlePrint}
              className="flex items-center space-x-2 px-4 min-h-[44px] bg-white hover:bg-stone-100 text-[#1e1b1b] font-bold text-xs rounded-sm border border-stone-300 uppercase tracking-wider transition cursor-pointer"
            ...
            <button
              onClick={onClose}
              className="p-2.5 min-h-[44px] min-w-[44px] rounded-full bg-white hover:bg-stone-100 text-stone-600 hover:text-[#1e1b1b] border border-stone-300 transition flex items-center justify-center cursor-pointer"
        ...
        <div className="no-print bg-white border border-stone-300 rounded-2xl p-4 sm:p-5 w-full max-w-4xl mb-6 space-y-3 text-xs shadow-md">
          <div className="flex items-center space-x-2 text-[#4a5d4a] font-bold border-b border-stone-200 pb-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
            <div>
              ...
              <select
                ...
                className="w-full bg-[#fcf9f8] border border-stone-300 rounded-sm px-3 py-2 text-[#1e1b1b] font-semibold focus:outline-none focus:border-[#4a5d4a] min-h-[44px]"
              >
            ...
            <div>
              <label className="text-[#1e1b1b]/80 font-bold block mb-1">Monto Estimado ($ ARS):</label>
              <input
                type="text"
                value={customPrice}
                onChange={(e) => setCustomPrice(e.target.value)}
                placeholder="Ej: 380.000"
                className="w-full bg-[#fcf9f8] border border-stone-300 rounded-sm px-3 py-2 text-[#4a5d4a] font-mono font-bold focus:outline-none focus:border-[#4a5d4a] min-h-[44px]"
              />
            </div>

            <div>
              <label className="text-[#1e1b1b]/80 font-bold block mb-1">Nombre del Comercio / Empresa:</label>
              <input
                type="text"
            ...
            <div>
              <label className="text-[#1e1b1b]/80 font-bold block mb-1">Atención / Contacto:</label>
              <input
                type="text"
                value={clientContact}
            ...
            <div>
              <label className="text-[#1e1b1b]/80 font-bold block mb-1">Días de Entrega Estimados:</label>
              <input
                type="text"
                value={deliveryDays}
          ...
          <div>
            <label className="text-[#1e1b1b]/80 font-bold block mb-1">Observaciones & Aclaraciones Especiales:</label>
            <input
              type="text"
              value={specialNotes}
        ...
        <div className="printable-proposal bg-white text-slate-900 w-full max-w-4xl rounded-2xl p-6 sm:p-10 shadow-2xl space-y-6 font-sans border-2 border-slate-300">
          ...
          {/* Top Sage Accent Bar */}
          <div className="h-2 bg-[#4a5d4a] rounded-t-xl -mt-6 sm:-mt-10 -mx-6 sm:-mx-10" />

          {/* Header Branding Banner */}
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b-2 border-stone-300 pb-5 gap-4">
            <div className="space-y-1">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-xs font-black text-[#4a5d4a] uppercase tracking-wide">
          ...
          <div className="bg-stone-50 rounded-2xl p-4 sm:p-5 grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-medium border border-stone-200">
            <div className="space-y-1">
            ...
            <div className="text-left sm:text-right space-y-1 border-t sm:border-t-0 pt-2 sm:pt-0 border-stone-200">
              <span className="text-[10px] font-black uppercase text-slate-500 tracking-wider block">Modalidad & Inversión Estimada:</span>
              <div className="text-base font-black text-[#4a5d4a]">{selectedPlan}</div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between border-b border-slate-200 pb-2">
              <span className="text-xs font-black uppercase tracking-wider text-[#4a5d4a]">
                1. Especificaciones del Sistema Seleccionado
              </span>
              <span className="px-2.5 py-0.5 rounded bg-[#4a5d4a]/15 text-[#4a5d4a] font-black text-[10px] uppercase">
                {currentSystem.badge}
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-2">
              {currentSystem.features.map((feat, idx) => (
                <div key={idx} className="flex items-start space-x-2 text-xs text-slate-800 bg-slate-50 p-2.5 rounded-xl border border-slate-200">
                  <CheckCircle2 className="w-4 h-4 text-[#4a5d4a] shrink-0 mt-0.5" />
                  <span>{feat}</span>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-3 pt-2">
            <span className="text-xs font-black uppercase tracking-wider text-[#4a5d4a] block border-b border-slate-200 pb-2">
              2. Cronograma de Implementación & Puesta en Marcha (~{deliveryDays} Días Hábiles)
            </span>

            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 text-xs">
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
                <span className="w-5 h-5 rounded-full bg-[#4a5d4a] text-white font-black text-[10px] flex items-center justify-center">01</span>
              ...
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
                <span className="w-5 h-5 rounded-full bg-[#4a5d4a] text-white font-black text-[10px] flex items-center justify-center">02</span>
              ...
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
                <span className="w-5 h-5 rounded-full bg-[#4a5d4a] text-white font-black text-[10px] flex items-center justify-center">03</span>
              ...
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
                <span className="w-5 h-5 rounded-full bg-[#4a5d4a] text-white font-black text-[10px] flex items-center justify-center">04</span>
          ...
          {/* Guarantee & Terms */}
          <div className="p-3.5 rounded-xl bg-stone-50 border border-stone-300 text-xs space-y-1">
